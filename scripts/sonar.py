...
"""
From: https://github.com/pvazteixeira/multibeam/blob/feature/mrf/sonar.py
"""
...
class Sonar:
  ...
  def __computeLookUp__(self, resolution):
   ...
   """
    ...
    # print 'Height:',height, self.height
  ...
  def toCart(self, ping, width=320, bg=0.0):
    ...
    Note: some conversion performance values as a function of resolution: 
      ...
      0.03 m/px    1ms
   """
    image_cart = np.zeros((self.height, self.width)) 
    image_polar = np.copy(ping)
    image_polar[0,0] = bg
    # print 'polar:',image_polar.shape # 498, 512
    # print 'row',np.max(self.row_polar.flatten())
    # print 'col',np.max(self.col_polar.flatten())
    # print 'cart',image_cart.shape  # 500, 907
    # print 'row',np.max(self.row_cart.flatten())
    # print 'col',np.max(self.col_cart.flatten())
    image_cart[self.row_cart.flatten(), self.col_cart.flatten()] = image_polar[self.row_polar.flatten(), self.col_polar.flatten()]
